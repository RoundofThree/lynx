...
class TransactionsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should not get show if not logged in" do
    ...
    assert_redirected_to sign_in_url  # TODO: check routes!!!!!!!!!!!!!!!!!!
  end
  ...
  test "create transaction without account_id should fail" do
    assert_no_difference 'Transaction.count' do
      post transactions_url, params: { transaction: { payer_account: "", payee_account_number: "12345678901234", payee_fullname: "Nyx", amount: 1.0, currency: "£" } }
    end
  ...
  test "create transaction with insufficient balance account should fail" do
    payer_account = accounts(:one)
    assert_no_difference 'Transaction.count' do
      post transactions_url, params: { transaction: { payer_account: payer_account, payee_account_number: "12345678901234", payee_fullname: "Nyx", amount: 1000000.0, currency: "£" } }
    end
    assert_redirected_to new_transaction_url
  end
  ...
  test "create transaction without valid payee account number should fail" do
    payer_account = accounts(:one)
    assert_no_difference 'Transaction.count' do
      post transactions_url, params: { transaction: { payer_account: payer_account, payee_account_number: "1234", payee_fullname: "Nyx", amount: 1000000.0, currency: "£" } }
    end
    assert_redirected_to new_transaction_url
  end
