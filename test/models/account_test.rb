...
class AccountTest < ActiveSupport::TestCase
  test 'account with payer account, balance 1000.10, account number 12345678901234, cvv 123, expiry date Janurary 2022 should be valid' do
    user = users(:have_two_accounts)
    account = Account.new(user: user, balance: 1000.10, account_number: '12345678901234', cvv: '123',
                          expiry_date: DateTime.new(2022, 1, -1), currency: 'GBP')
    assert account.valid?
  end

  test 'account with account number with not exactly 14 characters should be invalid' do
    user = users(:have_two_accounts)
  ...
  test 'account with no user should be invalid' do
    account = Account.new(user: nil, balance: 1000.10, account_number: '12345678901234', cvv: '123',
                          expiry_date: DateTime.new(2022, 1, -1), currency: 'GBP')
    assert_not account.valid?
  end
  ...
  test 'an account of negative balance should be invalid' do
    user = users(:have_two_accounts)
    account = Account.new(user: user, balance: -10.00, account_number: '12345678901234', cvv: '123',
                          expiry_date: DateTime.new(2022, 1, -1), currency: 'GBP')
    assert_not account.valid?
  end
  ...
  test 'an account with cvv longer than 4-digit should be invalid' do
    user = users(:have_two_accounts)
    account = Account.new(user: user, balance: -10.00, account_number: '12345678901234', cvv: '123456',
                          expiry_date: DateTime.new(2022, 1, -1), currency: 'GBP')
    assert_not account.valid?
  end
  ...
  test 'an account with cvv less than 3-digit should be invalid' do
    user = users(:have_two_accounts)
    account = Account.new(user: user, balance: -10.00, account_number: '12345678901234', cvv: '12',
                          expiry_date: DateTime.new(2022, 1, -1), currency: 'GBP')
    assert_not account.valid?
  end
